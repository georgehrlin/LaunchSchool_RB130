...
class TodoListTest < Minitest::Test
  ...
  def test_done?
    if @list.select(&:done?).size == @list.size
...
# end
  ...
  def test_item_at
    assert_raises(IndexError) { @list.item_at(99_999) }
...
# end
  ...
  def test_done!
    @list.done!
    @list.select(&:done?).size == @list.size
  end

  def test_remove_at
    assert_raises(IndexError) { @list.remove_at(99_999).nil? }
    @list.remove_at(1)
  ...
  def test_select0
    ...
    assert_equal(answer.to_s, @list.select(&:done?).to_s)
  end

  def test_select1
    @list.mark_done_at(1)
    assert_equal([@todo2], @list.select(&:done?).to_a)
  end
